...
Median read quality:  {:.2}
    ...
    if detail > 1 {
        ...
        print_thresholds(&_read_lengths_unsorted, &_read_qualities_unsorted, &reads).expect("failed threshold print");
        if detail > 2 {
            // unsorted values
            print_top_ranking(&read_lengths, &read_qualities, &top).expect("failed top print");
        }
    }
...
fn print_thresholds(read_lengths: &Vec<u64>, read_qualities: &Vec<f32>, reads: &u64) -> Result<(), Error> {
    ...
    let l_thresholds: Vec<u64> = vec![200, 500, 1000, 5000, 10000, 30000, 50000, 100000, 1000000];
    ...
    for (l, q) in read_lengths.iter().zip(read_qualities.iter()){ // must be unsorted
        for tq in &q_thresholds {
            if q > &tq {
                let qdata = q_threshold_data.entry(tq.to_string()).or_insert(vec![0, 0]);
                qdata[0] += 1;
                qdata[1] += l;
            }
        }
        for tl in &l_thresholds {
            if l > &tl {
                let ldata = l_threshold_data.entry(tl.to_string()).or_insert(vec![0, 0]);
                ldata[0] += 1;
                ldata[1] += l;
            }
        }
    ...
    let qkeys: Vec<String>  = q_threshold_data.keys().cloned().collect();
    let lkeys: Vec<String> = l_threshold_data.keys().cloned().collect(); 
    let mut qkeys: Vec<f32> = qkeys.iter().map(|x| x.parse::<f32>().unwrap() ).collect();
    let mut lkeys: Vec<u64> = lkeys.iter().map(|x| x.parse::<u64>().unwrap() ).collect();
    ...
    for k in qkeys.iter() {
        ...
        let percent_reads = (data[0] as f64 / *reads as f64)*100 as f64;
    ...
    for k in lkeys.iter() {
        ...
        eprintln!(">{:<9}: {:} ({:.2}%) {:}", k.to_formatted_string(&Locale::en), nreads, percent_reads, bp);
    }
    eprintln!("");
